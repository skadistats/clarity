...
#### Scenario: Interface is internal to S2NestedArrayEntityState
- **WHEN** `S2NestedEntityState` is used in the codebase
- **THEN** it is only referenced by `S2NestedArrayEntityState` and its inner `Entry` class
- **AND** it is NOT implemented by `S2TreeMapEntityState` or any other EntityState implementation

### Requirement: S2NestedArrayEntityState owns its traversal
`S2NestedArrayEntityState` SHALL implement `EntityState` directly. It SHALL provide its own `applyMutation(FieldPath, StateMutation)` that traverses the Field hierarchy via `field.getChild(idx)` and the Entry hierarchy via `entry.sub(idx)`. At the leaf, it SHALL dispatch on the `StateMutation` type.

#### Scenario: applyMutation with WriteValue
- **WHEN** `applyMutation(fp, WriteValue(value))` is called on S2NestedArrayEntityState
...
- **WHEN** `applyMutation(fp, ResizeVector(count))` is called on S2NestedArrayEntityState
...
- **WHEN** `applyMutation(fp, SwitchPointer(newSerializer))` is called on S2NestedArrayEntityState
...
- **WHEN** `getValueForFieldPath(fp)` is called on S2NestedArrayEntityState
- **THEN** it traverses the Field+Entry hierarchy and returns the value at the leaf
- **AND** it does NOT call `field.getValue(S2NestedEntityState, int)`
...
- **WHEN** `S2NestedArrayEntityState` is used as the S2 entity state
...
When `S2NestedArrayEntityState` releases an `EntryRef`, it SHALL also release every `EntryRef` transitively reachable through that Entry's `state[]` array, returning all of their slots to the freelist. No Entry SHALL remain in `entries` after its sole navigation path from the root has been removed.
...
#### Scenario: write on S2NestedArrayEntityState dispatches on leaf kind
...
### Requirement: S2NestedArrayEntityState mutates state in place during readFields

`S2FieldReader.readFieldsFast` SHALL mutate `S2NestedArrayEntityState` in place as each field is decoded, via `state.write(fp, decoded)`. No staging of mutations into `FieldChanges.mutations[]` SHALL occur on the fast path.

Packet-level atomicity is provided by the `entity-update-commit` capability: `readFields` throwing aborts the replay run; `queuedUpdates` are cleared in the `finally` block; no state rollback is attempted. `copy()` is eager (see `S2NestedArrayEntityState.copy() is an eager deep copy`) and is invoked only at baseline materialization and consumer-side snapshot points, not for per-packet atomicity.

#### Scenario: Fast-path decode mutates S2NestedArrayEntityState immediately

- **WHEN** `readFieldsFast` processes a field whose state is `S2NestedArrayEntityState`
...
- **WHEN** a packet decode throws after `S2NestedArrayEntityState` has been partially mutated in place
...
- **WHEN** `copy()` is invoked on a S2NestedArrayEntityState
