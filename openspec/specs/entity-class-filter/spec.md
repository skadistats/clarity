# entity-class-filter Specification

## Purpose

Opt-in per-class entity filter on the runner that lets consumers drop selected `DTClass`es entirely from Java-side materialization while keeping the wire-format bitstream cursor aligned. Filtered entities consume their CREATE/UPDATE/DELETE bits exactly as a non-filtered parser would, but allocate no `Entity`, write no state, fire no listeners, and are invisible to every entity-collection query.
