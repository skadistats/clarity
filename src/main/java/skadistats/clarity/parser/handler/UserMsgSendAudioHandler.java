package skadistats.clarity.parser.handler;

import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

import skadistats.clarity.match.Match;
import skadistats.clarity.model.UserMessage;
import skadistats.clarity.parser.Handler;
import skadistats.clarity.parser.HandlerHelper;
import skadistats.clarity.parser.RegisterHandler;

import com.dota2.proto.Usermessages.CUserMsg_SendAudio;

@RegisterHandler(CUserMsg_SendAudio.class)
public class UserMsgSendAudioHandler implements Handler<CUserMsg_SendAudio> {

    private final Logger log = LoggerFactory.getLogger(getClass());
    
    @Override
    public void apply(int peekTick, CUserMsg_SendAudio message, Match match) {
        HandlerHelper.traceMessage(log, peekTick, message);
        match.getUserMessages().add(UserMessage.build(message, match));
    }

}
