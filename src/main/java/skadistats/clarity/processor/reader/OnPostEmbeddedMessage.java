...
@UsagePointMarker(value = UsagePointType.EVENT_LISTENER, parameterClasses = { GeneratedMessage.class, BitStream.class }, dynamicParameters = true)
public @interface OnPostEmbeddedMessage {
    ...
    final class Event extends skadistats.clarity.event.Event<OnPostEmbeddedMessage> {
        ...
        public Event(Runner runner, Class<OnPostEmbeddedMessage> eventType, Set<EventListener<OnPostEmbeddedMessage>> listeners) {
        ...
        public void raise(GeneratedMessage msg, BitStream bs) {
            var bucket = byClass.get(msg.getClass());
            if (bucket != null) {
                for (var e : bucket) {
                    try {
                        e.listener.invoke(msg, bs);
                    } catch (Throwable t) {
