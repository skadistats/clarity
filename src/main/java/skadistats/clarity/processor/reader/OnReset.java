...
public @interface OnReset {
    ...
    final class Event extends skadistats.clarity.event.Event<OnReset> {
        private final Listener[] typedListeners;

        public Event(Runner runner, Class<OnReset> eventType, Set<EventListener<OnReset>> listeners) {
        ...
        public void raise(Demo.CDemoStringTables packet, ResetPhase phase) {
            for (int i = 0; i < typedListeners.length; i++) {
                try {
                    typedListeners[i].invoke(packet, phase);
                } catch (Throwable t) {
